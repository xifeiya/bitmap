...
#include "storage/relfilelocator.h" /* for RelFileLocator */

static void forget_incomplete_insert_bitmapwords(RelFileLocator node,
...
static void
log_incomplete_insert_bitmapwords(RelFileLocator node,
					  xl_bm_bitmapwords* newWords)
{
	int					lastTids_size;
	int					cwords_size;
...
static void
forget_incomplete_insert_bitmapwords(RelFileLocator node,
						 xl_bm_bitmapwords* newWords)
{
	ListCell* l;
	...
	foreach (l, incomplete_actions)
	{
		...
		if (RelFileLocatorEquals(node, action->bm_node) &&
 ...
 */
static void
_bitmap_xlog_newpage(XLogRecPtr lsn, XLogReaderState *record)
{
	...
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		...
		PageSetLSN(page, lsn);
		/* PageSetTLI is no longer needed in PostgreSQL 17 */
...
void
bitmap_redo(XLogRecPtr lsn, XLogReaderState *record)
{
