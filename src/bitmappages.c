...
_bitmap_init_buildstate(Relation index, BMBuildState *bmstate)
{
    ...
    {
	Oid typid = TupleDescAttr(bmstate->bm_tupDesc, attno)->atttypid;
	...
	for (attno = 0; attno < bmstate->bm_tupDesc->natts; ++attno)
	{
	    ...
	    Oid eq_opr; /* equality operator */
	    Oid atttypid = TupleDescAttr(bmstate->bm_tupDesc, attno)->atttypid; /* Get the equality operator's function */
        get_sort_group_operators(atttypid, false, true, false, NULL, &eq_opr, NULL, NULL);
     ...
     */	
    bmstate->use_wal = XLogArchivingActive() && !index->rd_islocaltemp;
...
_bitmap_init(Relation index, bool use_wal)
{
    ...
    Oid lovHeapId;  /* LOV heap id */
    Oid lovIndexId; /* LOV index id */
    ...
    /** 
     * before initializing the META page, we need to create the LOV heap and index.
     */
    _bitmap_create_lov_heapandindex(index, &lovHeapId, &lovIndexId);
    /**
     * allocate the first LOV item
     */
    ...
    metapage = (BMMetaPage) PageGetContents(page);
     /* Set the LOV heap and index ids */
    metapage->bm_lov_heapId = lovHeapId;
    metapage->bm_lov_indexId = lovIndexId;
    ...
    /* WAL disabled: skipping _bitmap_log_metapage */
    /* if (use_wal)
    ...
    elog(NOTICE, "DEBUG: Current Memory Context Name: %s", CurrentMemoryContext->name);
    ...
    /* WAL disabled: skipping _bitmap_log_lovitem */
    /* if(use_wal)
