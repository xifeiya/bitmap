...
#include "utils/snapshot.h" /* for SnapshotAny */
			 ...
			 uint64 tidnum, bool use_wal)
{
		  ...
		  (FILL_LENGTH(lovItem->bm_last_compword) == 1))))
	{
		...
		/* WAL disabled: skip logging last words */
		/* if (use_wal)
					...
					bool use_wal)
{
	...
	if (!IS_FILL_WORD(bitmap->hwords, wordNo))
	{
		...
		/* WAL disabled: skip logging updateword */
		/* if (use_wal)
	...
	/* WAL disabled: skip logging updatewords */
	/* if (use_wal)
			   ...
			   Buffer *bitmapBufferP, uint64 *firstTidNumberP)
{
	...
	*firstTidNumberP = 1;
	 ...
	 * So we error out.
	 */
	    elog(ERROR, "cannot find the bitmap page containing tid=%llu",
		    (unsigned long long) tidnum);
}
...
verify_bitmappages(Relation rel, BMLOVItem lovitem)
{
	...
	while (BlockNumberIsValid(nextBlockNo))
	{
		...
		if (bitmapOpaque->bm_last_tid_location != tidnum)
			elog(ERROR, "bm_last_tid_location=%llu, tidnum=%llu",
				 (unsigned long long)bitmapOpaque->bm_last_tid_location, (unsigned long long)tidnum);
							  ...
							  BMTIDBuffer* buf, bool use_wal)
{
	...
	while (numFreeWords < buf->curword - buf->start_wordno)
	{
		...
		if (use_wal)
		{
			/* WAL disabled: skipping _bitmap_log_bitmapwords */
		...
		elog(NOTICE,"[_bitmap_write_new_bitmapwords] CP1 (+=) : buf->start_wordno = %d , words_written = %llu"
			   ,buf->start_wordno,(unsigned long long)words_written
			 );
#endif		
	...
	if (use_wal)
	{
		...
		elog(NOTICE,"[_bitmap_write_new_bitmapwords] CP2 (+=) : buf->start_wordno = %d, words_written = %llu"
			 ...
			 "\n\tlovItem->bm_last_tid_location = %llu"
			   ,buf->start_wordno,(unsigned long long)words_written
			   ,(unsigned long long)lovItem->bm_last_setbit
			   ,(unsigned long long)lovItem->bm_last_tid_location
			 );
#endif		
	buf->start_wordno += words_written;
...
_bitmap_write_bitmapwords(Buffer bitmapBuffer, BMTIDBuffer *buf)
{
	...
	Assert(buf->tmp_hwords != NULL);
	Assert(num_hwords + 1 <= buf->tmp_hwords_cap);

	hwords = buf->tmp_hwords;
	MemSet(hwords, 0, (num_hwords + 1) * sizeof(BM_WORD));
			   ...
			   OffsetNumber *lovOffsetP, bool use_wal)
{
	...
	*lovBlockP = BufferGetBlockNumber(currLovBuffer);
	...
	/* Add the block number and offset number to the LOV item */
			...
			BMBuildState *state, BlockNumber lov_block, OffsetNumber off)
{
	...
	else
	{
		...
		buf->tmp_hwords_cap = BM_MAX_NUM_OF_HEADER_WORDS + 1;
		buf->tmp_hwords = palloc0(buf->tmp_hwords_cap * sizeof(BM_WORD));
		...
		elog(NOTICE,"[buf_add_tid] create new buf - CP1"
			 ...
			 "\n\tlast_compword = %u"
			 "\n\tlast_word = %d"
			   ,(unsigned long long)buf->last_tid
			 ,buf->last_compword
			 ,buf->last_word
			 );
#endif
			 ...
			 uint64 tidnum, BMTIDBuffer *buf, bool use_wal)
{
	...
	if (buf->num_cwords == 0)
	{
		...
		if (use_wal)
		{
			/* _bitmap_log_bitmap_lastwords WAL相关暂不实现 */
			...
			IndexScanDesc scanDesc, bool use_wal)
{
	...
	buf_free_mem(rel, &buf, lovBlock, lovOffset, use_wal, true);
...
void
_bitmap_buildinsert(Relation index, ItemPointer ht_ctid, Datum *attdata, 
				 ...
				 bool *nulls)
{
	...
	for (attno = 0; attno < tupDesc->natts; attno++)
	{
		RegProcedure	opfuncid = InvalidOid;
		ScanKey			scanKey = scanKeys + attno;
		Oid 			eq_opr = InvalidOid; /* equality operator */

		/* Get the equality operator OID */
		// Determine equality operator function for this attribute
		// scanKey = (ScanKey) (((char *)scanKeys) + attno * sizeof(ScanKeyData));
		get_sort_group_operators(TupleDescAttr(tupDesc, attno)->atttypid,
					 false, true, false, NULL, &eq_opr, NULL, NULL);
		...
		ScanKeyEntryInitialize(scanKey,
						/* flags */ 0,
						/* attributeNumber */ attno + 1,
						/* strategy */ BTEqualStrategyNumber,
						/* subtype (collation) */ InvalidOid,
						/* collation */ InvalidOid,
						/* procedure */ opfuncid,
						/* argument - set below */ (Datum) 0);

		if (nulls[attno])
		{
			scanKey->sk_flags |= SK_ISNULL;
			// scanKey->sk_argument |= SK_SEARCHNULL;
		}
		else
		{
	...
	for (attno = 0; attno < tupDesc->natts; attno++)
	{
		...
		elog(NOTICE, "Insert ScanKey[%d]: att=%d flags=%d strategy=%d func=%u arg=%ld",
			attno, scanKey->sk_attno, scanKey->sk_flags,
...
void _debug_view_2(BMTIDBuffer *x, const char *msg) 
{
  ...
  elog(NOTICE,"[_debug_view_BMTIDBuffer] %s"
	  ...
	  ,(unsigned long long)x->start_tid
	  ,(unsigned long long)x->last_tid
	   ,x->curword
	   ,x->num_cwords
	   ,x->start_wordno
  ...
  for (i=0; i < x->num_cwords; i+=8) {
		elog(NOTICE,"last_tids[%03x-%03x] = %08llu %08llu %08llu %08llu %08llu %08llu %08llu %08llu"
	 ,i,i+7
		 ,(unsigned long long)(x->last_tids)[i]
		 ,(unsigned long long)(x->last_tids)[i+1]
		 ,(unsigned long long)(x->last_tids)[i+2]
		 ,(unsigned long long)(x->last_tids)[i+3]
		 ,(unsigned long long)(x->last_tids)[i+4]
		 ,(unsigned long long)(x->last_tids)[i+5]
		 ,(unsigned long long)(x->last_tids)[i+6]
		 ,(unsigned long long)(x->last_tids)[i+7]
	 );
  }
}